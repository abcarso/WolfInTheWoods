...
public class FirstPersonController : MonoBehaviour
{
    ...
    public float hungerLossRate = 3.0f;
    ...
    void Start()
    {
        ...
        hungerBarWidth = screenWidth * hungerBarWidthPercent;
        hungerBarHeight = screenHeight * hungerBarHeightPercent;
        hungerBarBG.rectTransform.sizeDelta = new Vector3(hungerBarWidth, hungerBarHeight, 0f);
        hungerBar.rectTransform.sizeDelta = new Vector3(hungerBarWidth - 2, hungerBarHeight - 2, 0f);
    }
    ...
    private void Update()
    {
        ...
        if (enableHunger)
        {
            ...
            float hungerRemainingPercent = hungerRemaining / startingHunger;
            hungerBar.transform.localScale = new Vector3(hungerRemainingPercent, 1f, 1f);
        }
...
// Custom Editor
    ...
    public override void OnInspectorGUI()
    {
        ...
        GUILayout.Label("End Screen", new GUIStyle(GUI.skin.label)
        {
        ...
        EditorGUILayout.BeginHorizontal();
        EditorGUILayout.PrefixLabel(new GUIContent("Lose Screen Panel", "UI Panel to show when the game ends."));
        fpc.losePanel = (GameObject)EditorGUILayout.ObjectField(fpc.losePanel, typeof(GameObject), true);
        EditorGUILayout.EndHorizontal();
        ...
        #endregion
        #region Hunger
        GUILayout.Label("Hunger", new GUIStyle(GUI.skin.label) {
        alignment = TextAnchor.MiddleLeft,
        fontStyle = FontStyle.Bold,
        fontSize = 13
    }, GUILayout.ExpandWidth(true));

    fpc.enableHunger = EditorGUILayout.ToggleLeft(new GUIContent("Enable Hunger", "Determines if the player has hunger"), fpc.enableHunger);
    fpc.hungerLossRate = EditorGUILayout.FloatField(new GUIContent("Hunger Loss Rate", "How fast hunger depletes per second."), fpc.hungerLossRate);
    fpc.startingHunger = EditorGUILayout.FloatField(new GUIContent("Starting Hunger", "Initial hunger value when the game starts."), fpc.startingHunger);

    EditorGUI.indentLevel++;

    EditorGUILayout.BeginHorizontal();
    EditorGUILayout.PrefixLabel(new GUIContent("Bar BG", "UI Image for hunger bar background."));
    fpc.hungerBarBG = (Image)EditorGUILayout.ObjectField(fpc.hungerBarBG, typeof(Image), true);
    EditorGUILayout.EndHorizontal();

    EditorGUILayout.BeginHorizontal();
    EditorGUILayout.PrefixLabel(new GUIContent("Bar", "UI Image for hunger bar fill."));
    fpc.hungerBar = (Image)EditorGUILayout.ObjectField(fpc.hungerBar, typeof(Image), true);
    EditorGUILayout.EndHorizontal();

    fpc.hungerBarWidthPercent = EditorGUILayout.Slider(new GUIContent("Bar Width", "Width of the hunger bar relative to screen width."), fpc.hungerBarWidthPercent, 0.1f, 0.5f);
    fpc.hungerBarHeightPercent = EditorGUILayout.Slider(new GUIContent("Bar Height", "Height of the hunger bar relative to screen height."), fpc.hungerBarHeightPercent, 0.001f, 0.025f);
